...
- Вычисляет **Value Score** — цену, приведённую к качеству (Elo) и нормированную внутри категории так, что лидер получает 100
...
| **Scoring Engine** | `app/scoring.py` | Формула Value Score: цена, приведённая к качеству (Elo → эффективные $/1M), с нормировкой внутри категории; веса `k`/`γ` |
...
```
M_top_N   = median(top_n лучших по рейтингу)                # якорь категории
...
value     = 100 × (min(price_eff) / price_eff)^γ            # 0..100, 100 = лидер категории
```
...
- **`value`** — «сколько процентов от эффективности лидера категории».
  Ограничен сотней сверху, сравним **внутри одной категории и одного пресета**.
- Медиана берётся по **рейтингу**, а не по дате: `created` из OpenRouter — дата
...
- `k` и `γ` задаются **пресетами**. `k` — во сколько раз рейтинг искупает цену,
  **порядок строк задаёт только он**; `γ` растягивает шкалу value (`γ < 1`
